...
module.exports = {
  summary: async (req, res) => {
    try {
      const filter = req.user?.orgId ? { organization: req.user.orgId } : { ...req.orgFilter };

      const [companies, contacts, products, options, offers, makes, offersByType] = await Promise.all([
