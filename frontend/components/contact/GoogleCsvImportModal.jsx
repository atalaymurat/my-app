...
const ERROR_MESSAGES = {
  empty: "Dosya boş görünüyor. Google Kişiler'den dışa aktardığınız CSV dosyasını seçin.",
  type: "Lütfen .csv uzantılı bir Google Kişiler dosyası seçin.",
  headers: "CSV başlıkları tanınmadı. Google Kişiler CSV formatını kullandığınızdan emin olun.",
  rows: "Aktarılabilecek kişi bulunamadı. Satırlarda ad, e-posta veya telefon olmalı.",
  selected: "İçe aktarmak için en az bir geçerli kişi seçin.",
  backend: "Kişiler içe aktarılamadı. Dosyayı kontrol edip tekrar deneyin.",
  partial: "Bazı satırlar aktarılamadı. Başarılı kayıtlar kişi listesine eklendi.",
};
...
function buildPreviewRow(item) {
  const raw = item.raw;
  const displayName = [raw["First Name"], raw["Middle Name"], raw["Last Name"]]
    .map((value) => value?.trim())
    .filter(Boolean)
    .join(" ");
  const emails = collectNumberedValues(raw, "E-mail");
  const phones = collectNumberedValues(raw, "Phone");
...
export default function GoogleCsvImportModal({ open, onClose, onImported }) {
  ...
  const importSelected = async () => {
    ...
    setError("");
    try {
      const { data } = await axios.post("/api/contact/import/google-csv", {
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative z-10 w-full max-w-5xl max-h-[88vh] overflow-hidden rounded-xl border border-stone-700 bg-stone-900 shadow-2xl">
        <div className="flex items-center justify-between border-b border-stone-700 px-4 py-3">
          <div>
            <h2 className="text-sm font-bold text-stone-100">Google CSV Import</h2>
            <p className="text-xs text-stone-500">Google Kişiler CSV dosyasından kişi aktarın.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="flex flex-col gap-3 rounded-lg border border-stone-700 bg-stone-950/40 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold text-stone-300">
                {file ? file.name : "Google Kişiler CSV dosyası seçin"}
              </p>
