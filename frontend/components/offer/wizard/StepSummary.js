...
function InfoRow({ label, value }) {
  ...
  return (
    <div className="flex items-start justify-between gap-3 py-2 border-b border-stone-800 last:border-0">
      ...
      <span className="text-xs text-stone-300 font-semibold text-right">
        {value}
      </span>
    </div>
  )
...
export default function StepSummary({ onPrev, message, isSubmitting }) {
  ...
  const totalList = filled.reduce(
    (s, i) => s + (Number(i.priceList) || 0) * (i.quantity || 1),
    0,
  )
  const totalOffer = filled.reduce(
    (s, i) => s + (Number(i.priceOffer) || 0) * (i.quantity || 1),
    0,
  )
  const totalNet = filled.reduce(
    (s, i) => s + (Number(i.priceNet) || 0) * (i.quantity || 1),
    0,
  )
    ...
    <div className="flex flex-col gap-3">
            ...
            <div key={i} className="px-4 py-3">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="text-sm font-semibold text-stone-200 truncate capitalize">
                    {item.title}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  {item.priceOffer > 0 && (
                    <p className="text-sm font-bold text-amber-400">
                      {formPrice(item.priceOffer)} {item.currency}
                    </p>
                  )}
                  {item.quantity > 1 && (
              ...
              {item.selectedOptions?.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-1 pl-7">
                  {item.selectedOptions.map((o, oi) => (
                    <span
                      key={oi}
                      className="text-[10px] px-2 py-0.5 rounded-full bg-stone-800 border border-stone-700 text-stone-400"
                    >
                      {o.label}
                    </span>
                  ))}
      ...
      <div className="rounded-xl border border-stone-700 overflow-hidden">
        ...
        <div className="px-4 py-1">
          ...
          <InfoRow
            label="KDV"
            value={`%${values.vatRate} — ${values.showVat ? "Göster" : "Gizle"}`}
          />
          <InfoRow
            label="Toplamlar"
            value={values.showTotals ? "Göster" : "Gizle"}
          />
        </div>
      </div>
              ...
              <>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-base font-black text-white">
                    {formPrice(totalOffer * (1 + values.vatRate / 100))}{" "}
                    {currency}
                  </span>
                </div>
              </>
