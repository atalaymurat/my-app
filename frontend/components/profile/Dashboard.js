  ...
  return (
    <svg width={size} height={size} className="-rotate-90">
      ...
      <circle cx={size / 2} cy={size / 2} r={r} fill="none"
        stroke={color} strokeWidth={5} strokeDasharray={`${dash} ${circ}`}
...
function Block({ title, count, accent, actions, chart, pills, viewHref }) {
  ...
  const accentText = {
    ...
    emerald: "text-emerald-400", violet: "text-violet-400", stone: "text-stone-300",
  };
  const accentBorder = {
  ...
  return (
    <div className={`rounded-2xl border border-stone-800 border-l-4 ${accentBorder[accent]} bg-stone-950/80 overflow-hidden`}>
      {/* Header */}
      ...
      {actions?.length > 0 && (
        <div className="flex flex-wrap gap-2 px-4 pb-4">
          {actions.map((a) => <ActionBtn key={a.label} href={a.href} accent={accent}>{a.label}</ActionBtn>)}
        </div>
      )}
    </div>
  );
